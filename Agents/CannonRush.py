# Template from: https://github.com/BurnySc2/python-sc2/blob/develop/examples/protoss/cannon_rush.py
import os
import sys
...
class CannonRushBot(BotAI):
    async def on_step(self, iteration):
        ...
        # If we have no forge, build one near the pylon that is closest to our starting nexus
            ...
            if pylon_ready:
                if self.can_afford(UnitTypeId.FORGE):
                    await self.build(
                        UnitTypeId.FORGE, near=pylon_ready.closest_to(nexus)
                    )
        ...
        # If we have no cannons but at least 2 completed pylons, automatically find a placement location and build them near enemy start location
            ...
            if self.structures(UnitTypeId.PYLON).ready.amount >= 2 and self.can_afford(
                UnitTypeId.PHOTONCANNON
            ):
                pylon = (
                    self.structures(UnitTypeId.PYLON)
                    .closer_than(20, self.enemy_start_locations[0])
                    .random
                )
        ...
        # Decide if we should make pylon or cannons, then build them at random location near enemy spawn
        elif self.can_afford(UnitTypeId.PYLON) and self.can_afford(
            UnitTypeId.PHOTONCANNON
        ):
            ...
            for _ in range(20):
                ...
                building = (
                    UnitTypeId.PHOTONCANNON
                    if self.state.psionic_matrix.covers(pos)
                    else UnitTypeId.PYLON
                )
