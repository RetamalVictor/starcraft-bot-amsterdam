...
class RLbot(BotAI):  # inhereits from BotAI (part of BurnySC2)
    ...
    async def build_forge(self, nexus):
        if (
            ...
            and self.structures(UnitTypeId.ASSIMILATOR).amount > 1
        ):  # if we don't have a forge:
    ...
    async def train_voidray(self):
        ...
        if self.structures(UnitTypeId.VOIDRAY).amount < 10 and self.can_afford(
            UnitTypeId.VOIDRAY
        ):
    ...
    async def build_cannon(self, nexus, nb_cannons=3):
        ...
        if (
            ...
            and self.structures(UnitTypeId.PHOTONCANNON).amount < nb_cannons
        ):
    ...
    async def intel(self):
        ...
        for nexus in self.townhalls:
            ...
            # print(nex_pos)
            cv2.circle(
                game_data, (int(nex_pos[0]), int(nex_pos[1])), 10, (0, 255, 0), -1
            )  # BGR
        ...
        for enemy_unit in self.enemy_units:
            if not enemy_unit.is_structure:
                ...
                if enemy_unit.name.lower() in worker_names:
                    cv2.circle(
                        game_data, (int(pos[0]), int(pos[1])), 1, (55, 0, 155), -1
                    )
                else:
                    cv2.circle(
                        game_data, (int(pos[0]), int(pos[1])), 3, (50, 0, 215), -1
                    )
